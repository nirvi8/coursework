...
csvs <- Sys.glob('*-tripdata.csv.csv')
trips <- data.frame()
for (csv in csvs) {
  print(csv)
  tmp <- read_csv(csv, na='\\N')
  
  # the date format changed to something ugly in 2020-09 which read_csv doesn't recognize as a datetime,
...
weather <- weather %>% mutate(TMAX = TMAX / 10) %>% mutate(TMIN = TMIN / 10)
